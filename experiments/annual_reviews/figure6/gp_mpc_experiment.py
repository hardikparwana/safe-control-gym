...
def plot_2D_comparison_with_prior(state_inds,
    ...
    while (xx<len(crun.state_horizon_cov)):
        ...
        casadi_horizon_states = crun.casadi_horizon_states[init_ind]
        jax_horizon_states = crun.jax_horizon_states[init_ind]
        # pdb.set_trace()
        ...
        for i in range(horizon_cov.shape[0]): # time horizon
            ...
            casadi_position = casadi_horizon_states[state_inds, i]
            jax_position = jax_horizon_states[state_inds, i]
            
            if i == 0:
                ax.plot(position[0], position[1], 'k.') #, label='GP-MPC Prediction horizon')
                # ax.scatter(casadi_position[0], casadi_position[1], edgecolor=cmap(cc[ind]), facecolor=cmap(cc[ind]), s=40)
                # ax.scatter(jax_position[0], jax_position[1], c='r')
